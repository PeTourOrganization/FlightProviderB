
package com.example.app.flightproviderb.flightdetails;

import jakarta.xml.bind.annotation.XmlAccessType;
import jakarta.xml.bind.annotation.XmlAccessorType;
import jakarta.xml.bind.annotation.XmlAttribute;
import jakarta.xml.bind.annotation.XmlRootElement;
import jakarta.xml.bind.annotation.XmlType;


/**
 * <p>Java class for anonymous complex type</p>.
 * 
 * <p>The following schema fragment specifies the expected content contained within this class.</p>
 * 
 * <pre>{@code
 * <complexType>
 *   <complexContent>
 *     <restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *       <attribute name="OwningPCC" use="required" type="{http://www.travelport.com/schema/common_v54_0}typeRef" />
 *     </restriction>
 *   </complexContent>
 * </complexType>
 * }</pre>
 * 
 * 
 */
@XmlAccessorType(XmlAccessType.FIELD)
@XmlType(name = "")
@XmlRootElement(name = "OwnershipChange", namespace = "http://www.travelport.com/schema/common_v54_0")
public class OwnershipChange {

    /**
     * New owning PCC of the PNR.
     * 
     */
    @XmlAttribute(name = "OwningPCC", required = true)
    protected String owningPCC;

    /**
     * New owning PCC of the PNR.
     * 
     * @return
     *     possible object is
     *     {@link String }
     *     
     */
    public String getOwningPCC() {
        return owningPCC;
    }

    /**
     * Sets the value of the owningPCC property.
     * 
     * @param value
     *     allowed object is
     *     {@link String }
     *     
     * @see #getOwningPCC()
     */
    public void setOwningPCC(String value) {
        this.owningPCC = value;
    }

}
