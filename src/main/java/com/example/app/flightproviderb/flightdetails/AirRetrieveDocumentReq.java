
package com.example.app.flightproviderb.flightdetails;

import java.util.ArrayList;
import java.util.List;
import jakarta.xml.bind.annotation.XmlAccessType;
import jakarta.xml.bind.annotation.XmlAccessorType;
import jakarta.xml.bind.annotation.XmlAttribute;
import jakarta.xml.bind.annotation.XmlElement;
import jakarta.xml.bind.annotation.XmlRootElement;
import jakarta.xml.bind.annotation.XmlType;


/**
 * <p>Java class for anonymous complex type</p>.
 * 
 * <p>The following schema fragment specifies the expected content contained within this class.</p>
 * 
 * <pre>{@code
 * <complexType>
 *   <complexContent>
 *     <extension base="{http://www.travelport.com/schema/common_v54_0}BaseReq">
 *       <choice>
 *         <element ref="{http://www.travelport.com/schema/air_v54_0}AirReservationLocatorCode" minOccurs="0"/>
 *         <element ref="{http://www.travelport.com/schema/common_v54_0}TicketNumber" maxOccurs="999" minOccurs="0"/>
 *         <element name="TCRNumber" type="{http://www.travelport.com/schema/air_v54_0}typeTCRNumber" maxOccurs="999" minOccurs="0"/>
 *       </choice>
 *       <attGroup ref="{http://www.travelport.com/schema/common_v54_0}attrLocatorInfo"/>
 *       <attribute name="ReturnRestrictions" type="{http://www.w3.org/2001/XMLSchema}boolean" />
 *       <attribute name="ReturnPricing" type="{http://www.w3.org/2001/XMLSchema}boolean" />
 *       <attribute name="RetrieveMCO" type="{http://www.w3.org/2001/XMLSchema}boolean" />
 *     </extension>
 *   </complexContent>
 * </complexType>
 * }</pre>
 * 
 * 
 */
@XmlAccessorType(XmlAccessType.FIELD)
@XmlType(name = "", propOrder = {
    "airReservationLocatorCode",
    "ticketNumber",
    "tcrNumber"
})
@XmlRootElement(name = "AirRetrieveDocumentReq")
public class AirRetrieveDocumentReq
    extends BaseReq
{

    /**
     * Provider: 1G,1V,1P.
     * 
     */
    @XmlElement(name = "AirReservationLocatorCode")
    protected AirReservationLocatorCode airReservationLocatorCode;
    /**
     * Provider: 1G,1V,1P.
     * 
     */
    @XmlElement(name = "TicketNumber", namespace = "http://www.travelport.com/schema/common_v54_0")
    protected List<String> ticketNumber;
    /**
     * Provider: 1G,1V,1P-The identifying number for a Ticketless Air Reservation.
     * 
     */
    @XmlElement(name = "TCRNumber")
    protected List<String> tcrNumber;
    /**
     * Will return a response which includes a set of restrictions associated with the document.
     * 
     */
    @XmlAttribute(name = "ReturnRestrictions")
    protected Boolean returnRestrictions;
    /**
     * Provider: 1G,1V,1P-Will return a response which includes the pricing associated with the ETR.
     * 
     */
    @XmlAttribute(name = "ReturnPricing")
    protected Boolean returnPricing;
    /**
     * When true, returns MCO Information. The default value is false.
     * 
     */
    @XmlAttribute(name = "RetrieveMCO")
    protected Boolean retrieveMCO;
    /**
     * Contains the Locator Code of the Universal Record that houses this reservation.
     * 
     */
    @XmlAttribute(name = "UniversalRecordLocatorCode")
    protected String universalRecordLocatorCode;
    /**
     * Contains the Provider Code of the provider that houses this reservation.
     * 
     */
    @XmlAttribute(name = "ProviderCode")
    protected String providerCode;
    /**
     * Contains the Locator Code of the Provider Reservation that houses this reservation.
     * 
     */
    @XmlAttribute(name = "ProviderLocatorCode")
    protected String providerLocatorCode;

    /**
     * Provider: 1G,1V,1P.
     * 
     * @return
     *     possible object is
     *     {@link AirReservationLocatorCode }
     *     
     */
    public AirReservationLocatorCode getAirReservationLocatorCode() {
        return airReservationLocatorCode;
    }

    /**
     * Sets the value of the airReservationLocatorCode property.
     * 
     * @param value
     *     allowed object is
     *     {@link AirReservationLocatorCode }
     *     
     * @see #getAirReservationLocatorCode()
     */
    public void setAirReservationLocatorCode(AirReservationLocatorCode value) {
        this.airReservationLocatorCode = value;
    }

    /**
     * Provider: 1G,1V,1P.
     * 
     * Gets the value of the ticketNumber property.
     * 
     * <p>This accessor method returns a reference to the live list,
     * not a snapshot. Therefore any modification you make to the
     * returned list will be present inside the JAXB object.
     * This is why there is not a <CODE>set</CODE> method for the ticketNumber property.</p>
     * 
     * <p>
     * For example, to add a new item, do as follows:
     * </p>
     * <pre>
     * getTicketNumber().add(newItem);
     * </pre>
     * 
     * 
     * <p>
     * Objects of the following type(s) are allowed in the list
     * {@link String }
     * </p>
     * 
     * 
     * @return
     *     The value of the ticketNumber property.
     */
    public List<String> getTicketNumber() {
        if (ticketNumber == null) {
            ticketNumber = new ArrayList<>();
        }
        return this.ticketNumber;
    }

    /**
     * Provider: 1G,1V,1P-The identifying number for a Ticketless Air Reservation.
     * 
     * Gets the value of the tcrNumber property.
     * 
     * <p>This accessor method returns a reference to the live list,
     * not a snapshot. Therefore any modification you make to the
     * returned list will be present inside the JAXB object.
     * This is why there is not a <CODE>set</CODE> method for the tcrNumber property.</p>
     * 
     * <p>
     * For example, to add a new item, do as follows:
     * </p>
     * <pre>
     * getTCRNumber().add(newItem);
     * </pre>
     * 
     * 
     * <p>
     * Objects of the following type(s) are allowed in the list
     * {@link String }
     * </p>
     * 
     * 
     * @return
     *     The value of the tcrNumber property.
     */
    public List<String> getTCRNumber() {
        if (tcrNumber == null) {
            tcrNumber = new ArrayList<>();
        }
        return this.tcrNumber;
    }

    /**
     * Will return a response which includes a set of restrictions associated with the document.
     * 
     * @return
     *     possible object is
     *     {@link Boolean }
     *     
     */
    public Boolean isReturnRestrictions() {
        return returnRestrictions;
    }

    /**
     * Sets the value of the returnRestrictions property.
     * 
     * @param value
     *     allowed object is
     *     {@link Boolean }
     *     
     * @see #isReturnRestrictions()
     */
    public void setReturnRestrictions(Boolean value) {
        this.returnRestrictions = value;
    }

    /**
     * Provider: 1G,1V,1P-Will return a response which includes the pricing associated with the ETR.
     * 
     * @return
     *     possible object is
     *     {@link Boolean }
     *     
     */
    public Boolean isReturnPricing() {
        return returnPricing;
    }

    /**
     * Sets the value of the returnPricing property.
     * 
     * @param value
     *     allowed object is
     *     {@link Boolean }
     *     
     * @see #isReturnPricing()
     */
    public void setReturnPricing(Boolean value) {
        this.returnPricing = value;
    }

    /**
     * When true, returns MCO Information. The default value is false.
     * 
     * @return
     *     possible object is
     *     {@link Boolean }
     *     
     */
    public Boolean isRetrieveMCO() {
        return retrieveMCO;
    }

    /**
     * Sets the value of the retrieveMCO property.
     * 
     * @param value
     *     allowed object is
     *     {@link Boolean }
     *     
     * @see #isRetrieveMCO()
     */
    public void setRetrieveMCO(Boolean value) {
        this.retrieveMCO = value;
    }

    /**
     * Contains the Locator Code of the Universal Record that houses this reservation.
     * 
     * @return
     *     possible object is
     *     {@link String }
     *     
     */
    public String getUniversalRecordLocatorCode() {
        return universalRecordLocatorCode;
    }

    /**
     * Sets the value of the universalRecordLocatorCode property.
     * 
     * @param value
     *     allowed object is
     *     {@link String }
     *     
     * @see #getUniversalRecordLocatorCode()
     */
    public void setUniversalRecordLocatorCode(String value) {
        this.universalRecordLocatorCode = value;
    }

    /**
     * Contains the Provider Code of the provider that houses this reservation.
     * 
     * @return
     *     possible object is
     *     {@link String }
     *     
     */
    public String getProviderCode() {
        return providerCode;
    }

    /**
     * Sets the value of the providerCode property.
     * 
     * @param value
     *     allowed object is
     *     {@link String }
     *     
     * @see #getProviderCode()
     */
    public void setProviderCode(String value) {
        this.providerCode = value;
    }

    /**
     * Contains the Locator Code of the Provider Reservation that houses this reservation.
     * 
     * @return
     *     possible object is
     *     {@link String }
     *     
     */
    public String getProviderLocatorCode() {
        return providerLocatorCode;
    }

    /**
     * Sets the value of the providerLocatorCode property.
     * 
     * @param value
     *     allowed object is
     *     {@link String }
     *     
     * @see #getProviderLocatorCode()
     */
    public void setProviderLocatorCode(String value) {
        this.providerLocatorCode = value;
    }

}
